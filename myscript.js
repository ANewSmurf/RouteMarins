...
async function getRaceId() {
    try {
        const raceKey = window.location.pathname.split("/").filter(Boolean)[0];
        if (!raceKey) {
            return undefined;
        }

        const response = await fetch("/races2.json");
        if (!response.ok) {
            throw new Error("Unable to load races2.json (HTTP " + response.status + ")");
        }

        const data = await response.json();
        const race = Array.isArray(data.races)
            ? data.races.find(function (item) {
                return item.url === raceKey;
            })
            : undefined;

        return race?.id;
    } catch (error) {
        console.warn("RouteMarins: unable to determine the Zezo race ID", error);
        return undefined;
    }
}
...
try {
    ...
    sendRoutePoints();
    raceIdPromise.then(function (raceId) {
        if (!/^\d+\.\d+$/.test(raceId || "")) {
            return;
        }
