...
async function getRaceId() {
    try {
        const raceKey = window.location.pathname
            .split("/")
            .filter(Boolean)[0];
        ...
        if (!response.ok) {
            throw new Error("Unable to load races2.json");
        }

        const data = await response.json();
        const race = data.races?.find(function (item) {
            return item.url === raceKey;
        });

        return race?.id;
    } catch (error) {
        console.error("Unable to determine the Zezo race ID:", error);
        return undefined;
    }
}
