...
function createCellPolar(value, el, row) {
  ...
  var url = "http://toxcct.free.fr/polars/?race_id=" + raceid + ".1&tws=" + ltws + "&twa="+ ltwa + "&utm_source=RouteMarins";
  cell.innerHTML = '<a href="' + url + '" target="_blank" rel="noopener noreferrer">' + value + '</a>';
  cell.title = 'Click to access toxcct\'s polar site with correct boat, TWS and TWA';
  row.appendChild(cell);
}
...
var exportGpx = function () {
    ...
    let xmlString = xml.end({
        ...
        gpxOutput = document.getElementById("gpxKmlOutput");
    gpxOutput.value = "";
    gpxOutput.value = xmlString;
    gpxOutput.select();
...
var exportkml = function () {
  ...
  let kmlString = kml.end({
      ...
      kmlOutput = document.getElementById("gpxKmlOutput");
  kmlOutput.value = "";
  kmlOutput.value = kmlString;
  kmlOutput.select();
  document.execCommand('copy');
};
document.getElementById("kmlExport").addEventListener("click", exportkml);
